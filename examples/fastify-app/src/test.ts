...
const APP_BASE = "http://127.0.0.1:3002";
...
const CASES: RouteCase[] = [
  ...
  { method: "GET", path: "/users/99?token=SECRET" },
  { method: "GET", path: "/zen" },
...
async function ensureServerUp(): Promise<void> {
  try {
    ...
    console.error(`         ${LIME}pnpm --filter example-fastify-app start${RESET}\n`);
    process.exit(1);
  }
}
...
async function main(): Promise<void> {
  console.log(`${LIME}${BOLD}sysko · example-fastify test${RESET}`);
  ...
  try {
    ...
    console.log(`  ${withRoute} with http.route resolved by Fastify plugin`);
    console.log(
