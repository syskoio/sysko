...
const APP_BASE = "http://localhost:3006";
...
async function ensureServerUp(): Promise<void> {
  try {
    ...
    console.error(`         ${LIME}pnpm --filter example-nestjs-app start${RESET}\n`);
    process.exit(1);
  }
}
...
async function main(): Promise<void> {
  console.log(`${LIME}${BOLD}sysko · example-nestjs test${RESET}`);
